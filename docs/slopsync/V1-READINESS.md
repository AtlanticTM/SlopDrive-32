...
- **OTA** — permanent HTTP escapee #1 (own token plane, rides next to the
  PsychicHttp work).
- **UITOKEN** — permanent HTTP escapee #2 (RFC-029 §4; ruled 2026-07-25).
...
| `/api/log` | **RFC-017** | spec-core log EVENT channel + ring-tail backfill; serial-quiet re-binds to first grant |
...
7. **`FALLBACK_LAYOUTS` in catalog.js** — deleted the day RFC-015 lands.
...
9. **The legacy :81 UiSocket plane entire** — protocol, senderTask, frame
   builders — once RFC-010 lands (its last load-bearing duty is the e-stop).
...
- **Read back input speed / accel / jerk — FOR DISPLAY.** Locate the
  ...
  **DOCTRINE CLAMP (operator ruling, 2026-07-25) — read before
  ...
  authored (RFC-008.5); **the MACHINE's job is to play back whatever it
  is fed as well as it possibly can.** Limits are shown to the operator,
  nothing more. Per RFC-006 the normative word is MAY, never SHOULD.
...
### Hub-side handoff sanity guard (RFC-008's "concrete first consequence")
...
OTA — **zero uncovered surfaces** once RFC-001…029 land. The AUTHOR pile
...
gated on the catalog-capacity restructure (RFC-009 action item, sized by
...
wants RFC-020's procedure pattern first), and **arbiter diagnostics**.
...
silent and total. Raised to 16384 (RFC-009's own sizing note projects 15–25 KB
