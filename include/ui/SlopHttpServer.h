...
// SlopHttpServer — the HTTP backend A/B seam (DOCTRINE.md §1: conditional
// compilation isolating driver implementations, no hardcoded coupling)
//
// Constraints:
//   ONE name, TWO implementations, selected at build time by
//   -DUSE_PSYCHIC_HTTP:
//
...
//                         silent never becomes readable — the measured
//                         4.8-5.0 s / 9.7 s HTTP_MAX_DATA_WAIT captures
//                         cannot happen. IdleGuardWebServer is ABSENT from
//                         this build (its header #errors if pulled in).
//                         Envs: sd32-psychic, sd32-psychic-ota.
//
//   Handler bodies stay untouched across both backends: src/ui/WebUI.cpp is
//   ~2200 lines of ~30 route handlers, and duplicating it per backend would
//   guarantee the two copies drift. They keep calling
//   _httpServer->send()/arg()/method()/sendHeader()/streamFile(), and only
//   this file changes what those calls mean; the Psychic side is a thin
//   request-scoped adapter presenting the exact WebServer-shaped surface the
//   handlers already use.
//
//   The Psychic adapter's one invariant: esp_http_server runs ALL request
//   handlers on a SINGLE task (a select() loop, not a thread pool), which is
//   why the adapter can park the current PsychicRequest*/PsychicResponse* in
//   members while a handler runs. Enabling PsychicHttp's ENABLE_ASYNC (async
//   worker threads) would break that invariant and silently cross-wire two
//   concurrent requests, so the build #errors if ENABLE_ASYNC is defined — do
//   not define it. The scope guard below is still save/restore-based, so a
//   nested dispatch (e.g. the 404 path re-entering) cannot orphan pointers.
