...
private:
  ...
  bool _enabled;
  // LOCAL PATCH (SlopDrive) -- RFC 6455 subprotocol selection. Empty means "no
  // preference": select the client's FIRST offered token. See
  // _selectSubprotocol() in the .cpp for the rationale.
  String _subprotocol;
  static String _selectSubprotocol(const String &offered, const String &accepted);
...
public:
  ...
  // LOCAL PATCH (SlopDrive). Restrict the handshake to ONE acceptable
  // subprotocol. When set, a client that does not offer it gets NO
  // Sec-WebSocket-Protocol header back, which per RFC 6455 4.2.2 means it MUST
  // fail the connection -- that is the point: a client asking for a version
  // this server does not speak must be told no, not humoured.
  void setSubprotocol(const char *proto) {
    _subprotocol = proto ? proto : "";
  }
  void setSubprotocol(const String &proto) {
    _subprotocol = proto;
  }
