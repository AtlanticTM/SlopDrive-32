...
| **A changed client** | An observed version change drops a paired device to recognised-pending. See the [change tripwire](../reference/dictionary.md#change-tripwire), and its honesty clause below |
...
| **Losing the stop control** | `stop` and `estop` are role-exempt. Authorisation can never take the stop away from the person in the room |
...
**Safety outranks authorisation.** Anybody may stop the machine; not everybody
...
| **Instrumentation** | libFuzzer with AddressSanitizer and UndefinedBehaviorSanitizer, with undefined-behaviour recovery disabled |
...
device's name, and trick an operator into authorising a stranger under a
