...
//
// Constraints:
//   Phase 3: real motion. Streamed 0x7B setpoints via
//   StreamedSetpointExecutor, gated behind the SAME homed/enabled discipline
//   FAS mode uses — nothing here ever commands motion on its own; the
//   arbiter is still the sole caller (MotorDriver.h friend grant), this
//   driver just hands arbiter intents to the executor as track() targets.
//   Homing itself (both styles) is Phase 4 — this phase only has the BENCH
//   forceHomeState() path to get outputs live for bring-up.
//
//   Native unit: ENCODER COUNTS, not FAS steps. mmToNative()/nativeToMm()
//   use AIM_ENC_COUNTS_PER_MM (config_api.h) — the wire unit of BOTH the
//   0x7B setpoint frame and the feedback registers 0x16/0x17, and it is
//   fixed silicon (32768 counts/motor-rev), independent of the e-gear
//   register 0x0B. A Configure-pane e-gear change can never rescale
//   Modbus-mode motion the way it rescales FAS mode's steps/mm.
//
//   Virtual mapping ("cmd frame" below = this driver's own native-counts
//   frame, home=0, front=NEGATIVE — identical sign convention to the
//   arbiter's dispatch and to AIMServoDriver's FAS steps):
...
public:
    ...
    // the sole-caller lock deliberately.
    ...
    // the very first setpoint sent is motionless (hard safety requirement:
    // never command motion before a live encoder seed exists). Refuses if the bus has never delivered a valid encoder
    ...
    // physically watching. Real homing (both styles) lands in Phase 4.
...
public:
    ...
    // read, in counts/s² instead of steps/s².
    ...
    // (handleApiServo talks to ServoModbus directly).
...
private:
    ...
    // Constructed bound to the same bus reference the driver itself holds.
    ...
    // re-trust a link that just dropped ~3-15 setpoint echoes in a row.
    ...
    // the profile's ultimate target.
    float _target_counts = 0.0f;

    // ---- streamToSteps() grit-cache (MUST be the first check) ---------------
    // Called up to ~1kHz from the stream sampler; skip re-planning entirely
    // when target/speed/accel are byte-identical to last time.
