...
<!-- ==========================================================
     ...
     Source of truth: docs/slopsync/examples/session-traces.md
...
that needs a rule this spec doesn't state is a spec bug** (SPEC [§17.3](conformance.md#s17-3)). Frame notation:
...
## E2 — Reconnect mid-motion: etag skip, reconcile, no silent control resume {#e2}
...
| 2 | hub | t=600 ms: deadman fires. Initiator-bound source ⇒ STOP (decel); safety(0x0003) latches the STOP bit with `cause=deadman`; ownership released; control-owner(0x0004) updates; `stop_latched` edge on safety-events(0x000E) | [§11.3](safety.md#s11-3), [§11.4](safety.md#s11-4), [§9.4](channels.md#s9-4) |
| 3 | h→* | safety + control-owner STATE broadcast to all remaining subscribers | [§9.1](channels.md#s9-1) |
| 4 | c→h | t=3 s: transport restores → HELLO{same instance_id, token, cached catalog_etag, standing wish-lists} | [§6.8](session.md#s6-8) |
| 5 | h | instance_id matches a session that is already gone — nothing to evict; a new session_id is issued | [§6.3](session.md#s6-3) |
| 6 | h→c | WELCOME{roles:1 (token valid), same boot_id, same etag, …} | [§6.3](session.md#s6-3), [§12.3](security.md#s12-3) |
| 7 | c | etag matches cache ⇒ **ready immediately, zero catalog bytes on the wire** | [§6.4](session.md#s6-4), [§6.8](session.md#s6-8) |
| 8 | h→c | retained STATE push including safety showing STOP / `cause=deadman` | [§9.1](channels.md#s9-1) |
| 9 | c | the client had a pending unacknowledged intent (set-speed 380) from before the blip. It does **not** retransmit; it compares desire (380) against the adopted config (360) — still wanted ⇒ fresh INTENT{intent_id:1 in the new session, value:380} | [§6.8](session.md#s6-8), [§9.3](channels.md#s9-3) |
...
| 11 | c | user taps "resume stream" → an activating intent for the streaming source (fresh, explicit) ⇒ granted, ownership reassigned, STOP clears per [§11.1](safety.md#s11-1) | [§6.8](session.md#s6-8), [§11.4](safety.md#s11-4), [§11.1](safety.md#s11-1) |
