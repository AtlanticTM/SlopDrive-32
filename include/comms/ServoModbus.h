...
//   accel, 0x05/0x06/0x07 gains, 0x09 DIR polarity, 0x0A/0x0B e-gear, 0x14
//   save flag (EEPROM commit — reprogramBaud() never writes it; see that
//   method's doc).
//   All register values are 16-bit two's-complement for signed fields;
//   CRC16 is Modbus RTU polynomial 0xA001 (reflected); the XY-G485
...
// registers actually answered (a drive variant may not implement 0x15+).
struct ServoConfig {
...
// counters are wired end-to-end so the UI has a bus-health chip from day one.
...
class ServoModbus {
public:
    // port: HardwareSerial reference (e.g. Serial1) — must be set up by the
    // caller with the right baud/pins/config BEFORE init().
    // addr: Modbus slave address (default 1 per datasheet).
    ...
    // and returns false, leaving the link exactly as it was.
    ...
    // by trusting the echo — Ground Truth Doctrine applies to drives too.
...
private:
    ...
    // the not-ready reprobe alternation in update().
    ...
    // at a time (gold-motor access pattern); 25ms spacing gives a full
    // telemetry refresh every ~0.3s (8 singles + the encoder pair slot).
    ...
    // which is what you need to debug wiring — no TX light at all means the
    // frames never leave the ESP32.
    ...
    // 19200 baud for a 6-register read, so there's zero risk of timeout.
    ...
    // POLL_REG_COUNT+1 the HI half in single-read fallback mode).
    ...
    // and the validator gates its verdicts on standstill anyway.
    ...
    // single-writer today), read back via getBusHealth() under the mux.
    ...
    // becomes encoder-follow instead of echo.
    ...
    // which is exactly what the bench needs to see to pick the next knob.
    ...
    // at 0x14. Committed as one snapshot under the spinlock.
    ...
    // exit, then touch the wire.
    // kind 0 = single FC 0x06 write (reg/val). kind 1 = FC 0x10 pair write to
    // 0x0C/0x0D (val = first word, val2 = second word) — bench position probe.
