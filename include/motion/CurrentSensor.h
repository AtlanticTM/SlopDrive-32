...
//
// Constraints:
//   Build-guarded behind DRIVER_AIM_SERVO. Sits on the DIRTY side behind an
//   ISO1640 I2C isolator; presents as a normal device on the Wire bus from
//   the S3's point of view. Sensorless homing depends on this reading (free
//   travel draws little current, a stall against the hard stop drives it
//   toward the ceiling). Wraps RobTillaart/INA228 — register map is NOT the
//   INA226, do not swap libs.
//   Address:     0x40 (A0/A1 -> GND)
//   Shunt:       5 mΩ, 2W
//   ADCRANGE:    0 -> ±163.84mV full scale -> 32.768 A full scale
//   SHUNT_CAL:   4096 (exact)
//   CURRENT_LSB: 62.5 µA
//   ALERT:       not wired — all limiting is polled in software.
...
public:
    ...
    // The homing loop watches the magnitude of this. Returns 0 if not ready.
    float readCurrentA();

    // Bus voltage in VOLTS — the actual 36V rail. Free telemetry, handy for the
    // bring-up sanity check (should read ~36V with the rail live).
    float readBusV();

    // Instantaneous bus power in WATTS (V×I product computed on-chip). Free —
    // the INA228 already has both operands, no extra math needed on our end.
    float readPowerW();
    ...
    // a hot board, which is worth knowing about before something lets go.
    float readDieTempC();

    // Raw shunt voltage in VOLTS — what the current measurement is actually
    // derived from. Mostly diagnostic (sanity-check the shunt/current math).
    float readShuntV();
    ...
    // just read the running total). resetPeaks() clears it back to zero.
    float readEnergyWh();
    ...
    // so a torn read is impossible and no mutex is needed for a display value.
    ...
    // the last stroke/home without watching the live number continuously.
    ...
    // Wh counter back at zero too. Call this at the start of a homing cycle.
    void  resetPeaks();
    ...
    // then anyway (homing owns the machine exclusively while it runs).
    void  poll() {
        readCurrentA();
        readBusV();
        readPowerW();
    ...
    // refresh RATE and the noise floor are decoupled on this chip.
    void  pollFast() {
        readCurrentA();
        readBusV();
    }
...
private:
    ...
    // Peak trackers — highest magnitude seen since boot / since resetPeaks().
