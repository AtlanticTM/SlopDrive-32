...
in `registry.yaml`), and `0x0080–0x7FFF` belong to the **device** (each hub
allocates its own, self-described by its catalog). This file is the
human-readable map; `registry.yaml` + the device catalog remain the wire
truth. The two tables below are GENERATED by `tools/gen_channel_map.py`
(`--check` verifies they match; never hand-edit inside a GENERATED marker
pair) -- everything else on this page is hand-maintained prose.
...
`0x1012` thermal (RFC-048 capability interfaces — this device has no
hardware behind either yet).
...
last time it changed; see `tools/gen_channel_map.py`'s `OLD_ID` comment).
