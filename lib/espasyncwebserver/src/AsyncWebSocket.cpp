...
// LOCAL PATCH (SlopDrive) -- RFC 6455 4.2.2 subprotocol selection.
//
// `offered`  the client's raw Sec-WebSocket-Protocol value: one or more tokens,
//            comma separated, optionally surrounded by whitespace.
// `accepted` what this server will agree to; empty means "no preference".
//
// Returns exactly ONE token, or an empty String meaning "refuse to negotiate"
// (the caller then omits the response header entirely).
//
// With no preference configured we return the client's FIRST token rather than
// the whole list. That keeps upstream's permissive spirit while still emitting
// a legal single-value response, so other users of this library do not suddenly
// start failing handshakes because of this patch.
String AsyncWebSocket::_selectSubprotocol(const String &offered, const String &accepted) {
  int start = 0;
  while (start <= (int)offered.length()) {
    int comma = offered.indexOf(',', start);
    if (comma < 0) {
      comma = offered.length();
    }
    int b = start, e = comma;
    while (b < e && isspace((unsigned char)offered[b])) {
      b++;
    }
    while (e > b && isspace((unsigned char)offered[e - 1])) {
      e--;
    }
    if (e > b) {
      const String token = offered.substring(b, e);
      if (!accepted.length()) {
        return token;   // no preference: first offered token wins
      }
      if (token == accepted) {
        return token;   // the one we speak was offered
      }
    }
    start = comma + 1;
  }
  return String();      // nothing acceptable was offered
}
...
void AsyncWebSocket::handleRequest(AsyncWebServerRequest *request) {
  ...
  if (request->hasHeader(WS_STR_PROTOCOL)) {
    // LOCAL PATCH (SlopDrive) -- replaces `// ToDo: check protocol`.
    //
    // Upstream echoed the client's ENTIRE Sec-WebSocket-Protocol header back
    // verbatim. That is wrong two ways. RFC 6455 4.2.2 requires the server to
    // select EXACTLY ONE subprotocol, so a comma-separated response is
    // malformed; and echoing whatever was asked for means the server claims to
    // speak any version a client names -- a client offering a future protocol
    // revision is told "yes" by a server that has never heard of it, and the
    // mismatch surfaces later as garbled frames instead of a clean refusal at
    // handshake time.
    const AsyncWebHeader *protocol = request->getHeader(WS_STR_PROTOCOL);
    const String selected = _selectSubprotocol(protocol->value(), _subprotocol);
    if (selected.length()) {
      response->addHeader(WS_STR_PROTOCOL, selected);
    }
    // No match -> header omitted deliberately. Per RFC 6455 the client MUST
    // then fail the connection, which is the refusal we want.
  }
  request->send(response);
}
