...
- **`ws_port` (46) and `ipv4` (47)** (RFC-046 item 3) are the hub's own WebSocket port and IPv4 address, present on every binding — including WS itself, closing the same "what does the hub believe its own endpoint is" gap for a WS client — but load-bearing specifically over BLE: a BLE-connected client reads them to perform the §13.1/§13.4 upgrade hop to WebSocket without any out-of-band discovery. **`0` means absent** for either key (no WS listener, or no IPv4 address) — a client MUST treat `0` as "not offered right now," never as a literal port or address.
...
|---|---|---|---|
| **Deadman** (§11.3) | a session that **owns an active motion source** | silence beyond `deadman_ms` (default 600, clamp 250–5000, negotiated at WELCOME) | session marked `STALE` (RFC-042 — see below), **not** torn down; ownership of the source released **unconditionally**, latching nothing (RFC-045) — a command-driven source has nothing left to execute and settles on its own (§11.3); a hub-autonomous source's `source.background_run` setting governs, default `false` (stops) |
...
Codes of note: `NORMAL_CLOSURE` (clean voluntary teardown, either direction), `SESSION_EVICTED`, `DUPLICATE_INSTANCE`, `READY_TIMEOUT`, `SLOT_RECLAIMED` (RFC-042, §6.6), `BLOB_REFUSED` (§4.5, client-sent), `REBOOTING` (§9.3). `DEADMAN_TIMEOUT` and `IDLE_REAPED` remain registered GOODBYE codes but the reference hub no longer emits either — silence produces no GOODBYE at all (staleness is not an ending).
...
22. **BLE GATT, UDP discovery, and cross-transport migration are specified with no reference implementation.** RFC-043/046 pin the BLE identity, advertising payload, `ws_port`/`ipv4` WELCOME keys, the UDP probe/reply, and the migration rule of §6.3 — but the reference hub (SlopDrive-32) implements only the WebSocket binding today. Treat all of it as a specified extension point, not as field-tested behavior, until a BLE `ITransport` and a UDP responder land.
23. **RENDERING.md's vocabulary (categories, ranks, archetypes, widget patterns, regions, units, action tags, value axes) is specified and registry-numbered, but not wired onto any real catalog entry.** RFC-048 landed the spec/registry text only; no catalog field carries `category`, `rank`, an archetype hint, a `unit` id, or a value-axis annotation yet, and no reference client builds pages from this derivation chain. Treat all of it as a specified extension point — the Completeness Doctrine's fallback rule (RENDERING.md §14d) is exactly what guarantees this can land in a later catalog evolution without breaking anything shipped today.
24. **Blob backpressure and completion (RFC-050) are specified, not yet shipped.** §8.4's backpressure table and `blob_chunks_in_flight` replace the earlier advisory "a hub MAY pace... MUST respect backpressure" wording the spec fresh-eyes panel found gave no binding-independent signal to code against; BLOB_DONE (`0x20`) replaces "the sender just stops and hopes" with a positive, idempotent completion signal. The reference hub predates all of it: it does not yet count in-flight chunks against the limit, does not abort a sustained-congestion transfer with NACK `BUSY`, and no reference client or hub emits BLOB_DONE. Implementation: post-batch hub work (§18's usual meaning of "specified, not shipped" — do not infer any of this from observed reference-hub behavior).
...
- **Teardown equivalence** across all six session-end paths (§6.9), with `session_loss` distinguished from `deadman`.
