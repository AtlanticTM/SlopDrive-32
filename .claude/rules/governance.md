...
| 2026-09-02 | Fleet-review rulings: (1) the non-blocking rule in `architecture.md` now states that a bounded sub-millisecond busy-wait with a measured hardware reason is not a delay; (2) two instance rulings under `cpp-safety.md` Concurrency, recorded on the beads and not as rule changes: the bounded mutex around the arbiter's driver call (sd-tki.4) and the C5 bridge's per-direction UART mutexes (sd-tki.11) are approved. | operator |
| 2026-09-03 | One motion backend: the RP2350 over the link is the only motion driver; the Modbus and step-and-direction motion drivers are retired (sd-4k1.8), the Modbus bus stays for configuration and the encoder audit behind a build switch, and the inert stream speed mode is retired on the wire (sd-4k1.9). `architecture.md` section 1 amended. Follows the port ruling of the same day: build the port whole, flash it, fix later (sd-4k1.4). | operator |
| 2026-09-02 | SlopMotion review rulings (docs/reviews/slopmotion-2026-09-02, epic sd-6b2): (1) amplitude shrinking to hold a deadline is ratified as the one exception to ceilings-are-clamps, with the amplitude budget as a floor; Stretch stays as the alternative contract; the other four infeasibility policies, centering, the bridge ratio and the soften knobs are deleted. (2) Every duration-carrying segment plans as WAVEFORM; the duration floor and sample synthesis are deleted. (3) One activity clock for every liveness test; a reset keeps the cadence. `motion-control.md` amended in the same commit. The runway band and the RP dead-stop stay as ruled (sd-jff). | operator |
...
| 2026-09-20 | Per-board PlatformIO projects: flagship board firmware lives in `flagship_<chip>/` as its OWN project, not an env in the machine repo. The ULP hook resolves one `$PROJECT_DIR/ulp` with no per-env override and does not exclude esp32s3, so a root `ulp/` would compile into S3 production firmware on any lib rebuild. `repo-layout.md` amended. `src/p4_probe/` moved to `flagship_p4/` and its env removed. | operator |
